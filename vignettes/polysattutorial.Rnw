% \VignetteIndexEntry{polysat version 1.5 Tutorial Manual}
% \VignetteDepends{combinat, methods, polysat}
...
\title{\textsc{polysat} version 1.5 Tutorial Manual}
...
and estimate allele frequencies and $F_{ST}$.  Due to the
...
console (note that combinat is only needed if the \texttt{Bruvo.distance} 
...
\node at (4, -7) [block] (simplefreq) {Use \texttt{simpleFreq} to
...
If your data are autopolyploid, also see \texttt{?meandistance.matrix2}
for another method of calculating inter-individual distances.  If you
...
Nei's $G_{ST}$ \cite{nei73,nei83}, and Jost's $D$ \cite{jost08}
...
The function \texttt{calcFst} takes the data frame produced by either
...
\item Clark, LV and Drauch Schreier, A. 2015.  Resolving microsatellite genotype
ambiguity in populations of allopolyploid and diploidized autopolyploid organisms
using negative correlations between alleles.  \emph{BioRxiv}.  DOI: 10.1101/020610
