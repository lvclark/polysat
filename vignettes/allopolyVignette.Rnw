...
``polysat version 1.6 Tutorial Manual''.  Additionally, there is a manuscript on 
...
problematic loci, which take longer).
...
or third parameter set (the two parameter sets that include the allele
swapping algorithm) works best.  Loc4 worked best with the first, second,
or fourth parameter set.  For Loc 3 and Loc5,
...
We can see that for Loc1, Loc3, Loc4, and Loc5, the parameter set does not affect the 
...
be chosen for Loc1, and parameter set 1 for all other loci.  (Loc1 had an equal
amount of missing data for parameter sets 1 and 2, but less homoplasy for parameter
set 2.)
...
We can see that with parameter set 1, allele 233 was made homoplasious in Pop1 and
allele 227 was made homoplasious in Pop2, resulting in much homoplasy in the
