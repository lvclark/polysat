...
Null alleles at high frequency   & \texttt{rare.al.check = 0} \\
...
swapping algorithm) works best.  For the other loci,
...
We can see that for Loc3, Loc4, and Loc5, the parameter set does not affect the 
...
populations that it was not possible to merge them.  The parameter set 4 
