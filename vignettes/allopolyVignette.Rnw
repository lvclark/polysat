...
\texttt{processDatasetAllo} is a wrapper for \texttt{alleleCorrelations} and
\texttt{testAlGroups} that will test several parameter combinations across
...
which is also a list.  The list element called \texttt{significant.pos} indicates any
...
plot(x=seq(min(myassign$AlCorrArray[["Loc5", "Pop1"]]$heatmap.dist),
           max(myassign$AlCorrArray[["Loc5", "Pop1"]]$heatmap.dist), length.out=12),
...
allele frequencies from the data and use those to estimate $F_{ST}$.
